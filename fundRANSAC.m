function [matches_1, matches_2] = fundRANSAC(pts1, pts2, depth_img1, parameters)   
% using fundamental matrix for robust fitting
% using x'~H_inf*x+e'/z

minPtNum = parameters.minPtNum;  % minimal number of points to estimate H and e
iterNum = parameters.iterNum;  % maximum iterations
thDist = parameters.thDist;  % distance threshold

depth_pts1 = calcDepthofPoint(pts1, depth_img1);

%% perform coordinate normalization
[normalized_pts1, ~] = normalise2dpts([pts1; ones(1,size(pts1, 2))]);
normalized_pts1(3,:) = depth_pts1;
[normalized_pts2, ~] = normalise2dpts([pts2; ones(1,size(pts2, 2))]);
normalized_pts2 = normalized_pts2(1:2,:);
points = [normalized_pts1', normalized_pts2'];

fitmodelFcn = @(points)calcHande(points); % fit function 
evalmodelFcn = @(He, points)calcDistofHe(He, points);

rng(0);
[~, pro_inlierIdx] = ransac(points,fitmodelFcn,evalmodelFcn,minPtNum,thDist,'MaxNumTrials',iterNum);

inliers1 = pts1(:, pro_inlierIdx);
inliers2 = pts2(:, pro_inlierIdx);

matches_1 = inliers1;
matches_2 = inliers2;

% delete duplicate feature match
[~,  ind1] = unique(matches_1', 'rows');
[~,  ind2] = unique(matches_2', 'rows');
ind = intersect(ind1, ind2);
matches_1 = matches_1(:, ind);
matches_2 = matches_2(:, ind);

end

function [ He ] = calcHande(points) % estimate H_inf and e' via DLT

npts1 = points(:, 1:3)';
npts2 = points(:, 4:5)';

%% calculation the initial H0 and e0
Equation_matrix = zeros(2*size(npts1, 2), 12);
for i=1:size(npts1, 2)
    zi = npts1(3,i); xi = npts1(1,i); yi = npts1(2,i);
    xi_= npts2(1,i); yi_= npts2(2,i);
    tmp_coeff1 = [xi, yi, 1, 0,  0,  0, -xi*xi_, -yi*xi_, -xi_, 1/zi, 0,   -xi_/zi];
    tmp_coeff2 = [0,  0,  0, xi, yi, 1, -xi*yi_, -yi*yi_, -yi_, 0,   1/zi, -yi_/zi];
    Equation_matrix(2*i-1:2*i, :) = [tmp_coeff1; tmp_coeff2];
end

[~,~,v] = svd(Equation_matrix, 0);
norm_H0 = reshape(v(1:9, 12), 3, 3)';
norm_e0 = v(10:12, 12);
He = [norm_H0(:); norm_e0(:)];

end

function dist = calcDistofHe(He, points) % calculate the projective error

pts1 = points(:, 1:3)';
pts2 = points(:, 4:5)';

H = reshape(He(1:9),3,3);
e = He(10:12);

tmp1 = H(1,:)*[pts1(1:2,:); ones(1,size(pts1,2))]+e(1)./pts1(3,:);
tmp2 = H(2,:)*[pts1(1:2,:); ones(1,size(pts1,2))]+e(2)./pts1(3,:);
tmp3 = H(3,:)*[pts1(1:2,:); ones(1,size(pts1,2))]+e(3)./pts1(3,:);
mapped_pts2(1,:) = tmp1./tmp3;
mapped_pts2(2,:) = tmp2./tmp3;
dist = sqrt(sum((mapped_pts2-pts2).^2, 1));

end

